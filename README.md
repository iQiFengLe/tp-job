...
- **reaper**:扫 `waiting_receive`/`running` 做失败转移——worker 心跳超时(失联)→ `failed`(启动 `worker.warmup_seconds` 宽限内跳过此判定,给重启后 worker 重新注册留时间);`waiting_receive` 超 `receive_timeout_seconds`(worker 收到 `/run` 后迟迟不进入 `running`:繁忙/卡住/上报丢失)→ `failed`;执行超 `TimeoutSec` → `timeout`;均触发重试。后两项是"worker 繁忙但心跳正常、任务卡死"场景的核心兜底。无在线 worker 时(重启/全挂)实例保持 `queued` 退避重派、不判 failed(详见 `docs/design.md` §6.2/§7)。
