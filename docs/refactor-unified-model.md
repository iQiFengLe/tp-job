...
type Job struct {
    ...
    // —— 调度 ——
    ScheduleKind  string  `gorm:"type:varchar(16)"`   // cron | fix_rate | fix_delay | delay | run_at | manual
...
| `manual` | — | task_type=normal / PowerJob API |
...
- `powerjob`:`POST {workerAddress}/taskTracker/runJob` body `ServerScheduleJobReq`(`protocol/powerjob` translator 构造)
